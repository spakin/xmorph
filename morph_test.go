...
func TestMorph25NRGBA(t *testing.T) {
	...
	exp := []byte{0x41, 0x6c, 0xa5, 0xf7, 0x5f, 0xc5, 0x85, 0xaa, 0x2b,
		0x87, 0x93, 0x57, 0xf, 0x22, 0x31, 0x55, 0xe, 0xe5, 0x51,
		0x1a, 0x31, 0x6b, 0x56, 0x2, 0x93, 0x3b, 0xff, 0x56, 0xf6,
		0xf, 0x36, 0x31}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph50NRGBA(t *testing.T) {
	...
	// Compare the image's hash value to an expected value.
	exp := []byte{0x48, 0x4e, 0x64, 0x4f, 0x64, 0x68, 0x2b, 0x22, 0xd,
		0x52, 0x95, 0xb5, 0xc9, 0x89, 0x4f, 0xa4, 0x1, 0xc6, 0xa6,
		0xfd, 0xa6, 0x8a, 0x9b, 0x7e, 0xfa, 0x8d, 0xd1, 0x0, 0xf0,
		0x74, 0xaf, 0x3a}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph75NRGBA(t *testing.T) {
	...
	// Compare the image's hash value to an expected value.
	exp := []byte{0x7a, 0x94, 0x4d, 0xc8, 0x54, 0x1e, 0xf0, 0x1, 0x23,
		0xc0, 0x1d, 0xd7, 0x51, 0x4, 0x1d, 0x24, 0x8c, 0xb5, 0x80,
		0x35, 0x59, 0x55, 0x72, 0x2e, 0x5c, 0xe, 0x56, 0x8a, 0x85,
		0xb8, 0x8b, 0x1d}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph100NRGBA(t *testing.T) {
	...
	exp := []byte{0xf5, 0x35, 0x34, 0xd0, 0xbb, 0x1e, 0x35, 0x81, 0x90,
		0x66, 0x75, 0xe6, 0xa, 0xf0, 0x79, 0xd7, 0x24, 0xf4, 0xae,
...
func TestMorph50Alpha(t *testing.T) {
	...
	exp := []byte{0xa0, 0x79, 0xce, 0xf2, 0x65, 0x3f, 0xbd, 0x2b, 0x9f,
		0x3, 0x1e, 0x7e, 0x5a, 0x91, 0x4b, 0x73, 0x37, 0xdd, 0x94,
		0x39, 0x12, 0xfc, 0x59, 0x1, 0xa4, 0x5f, 0x94, 0x31, 0x1f,
		0x18, 0x9f, 0x7f}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph50RGBA(t *testing.T) {
	...
	exp := []byte{0x3f, 0x43, 0x6d, 0xd5, 0xc2, 0x9d, 0xfa, 0xc4, 0x4b,
		0xb1, 0xf7, 0xe7, 0x66, 0xc4, 0x6f, 0x48, 0x5a, 0x97, 0xb,
		0xe9, 0xe, 0xaa, 0x7f, 0xba, 0x9a, 0x6, 0xda, 0x81, 0x53,
		0x92, 0xaf, 0x17}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph50Gray16(t *testing.T) {
	...
	exp := []byte{0x77, 0xf8, 0x80, 0x8f, 0x46, 0xa9, 0x94, 0xe9, 0x33,
		...
		0x2, 0xb7, 0x89, 0x75, 0x7f, 0x8a, 0xfd, 0x5a, 0xd3, 0x73,
		0xb3, 0xec, 0x7c}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph50RGBA64(t *testing.T) {
	...
	exp := []byte{0x8c, 0x64, 0x4d, 0xbb, 0xac, 0x31, 0xc1, 0xaf, 0x7f,
		0xb5, 0x25, 0xcc, 0x34, 0x53, 0x20, 0x74, 0xe3, 0x5, 0xc6,
		0x67, 0xc4, 0x10, 0x3a, 0xc2, 0x1d, 0xd3, 0xe1, 0xc9, 0x5,
		0x67, 0x57, 0xf5}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
...
func TestMorph50NRGBACMYK(t *testing.T) {
	...
	// Compare the image's hash value to an expected value.
	exp := []byte{0x71, 0x41, 0x87, 0x79, 0x39, 0x36, 0xa, 0xf, 0x6b,
		0x73, 0xf9, 0x33, 0xc9, 0x21, 0x4a, 0x25, 0x37, 0x25, 0xc0,
		0x5a, 0x9c, 0x77, 0xe, 0xb7, 0xcc, 0x48, 0xfe, 0xb9, 0x57,
		0xb6, 0xbf, 0x40}
	hash := imageHash(t, morph)
	compareHashes(t, exp, hash)
}
